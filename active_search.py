...
train_arg.add_argument('--acive_search_steps', type=int, default= 300)
...
config, _ = get_config()
dir_ = str(config.dimension)+'D_'+'TSP'+str(config.graph_dimension)\
     +'_b'+str(config.batch_size)+'_e'+str(config.input_embed) \
    +'_n'+str(config.num_neurons)+'_s'+str(config.num_stacks)  \
    +'_h'+str(config.num_heads)+ '_q'+str(config.query_dim) +'_u'+str(config.num_units)\
    +'_c'+str(config.num_neurons_critic)+ '_lr'+str(config.lr_start) \
