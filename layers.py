...
def multihead_attention(inputs, graphs, num_units=None, num_heads=16, dropout_rate=0.1, is_training=True):
    with tf.variable_scope("multihead_attention", reuse=None):
        ...
        Graph_info = tf.tile(tf.layers.dense(graphs, graphs.get_shape().as_list()[-1], activation= tf.nn.relu), [num_heads,1,1])
        outputs = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1])) + Graph_info # num_heads*[batch_size, seq_length, seq_length]
        # Scale
...
def encode_seq(input_seq, graph_tensor, input_dim, num_stacks, num_heads, num_neurons, is_training, dropout_rate=0.):
    with tf.variable_scope("stack"):
        for i in range(num_stacks): # block i
            with tf.variable_scope("block_{}".format(i)): # Multihead Attention + Feed Forward
                input_seq = multihead_attention(input_seq, graph_tensor, num_units=input_dim, num_heads=num_heads, dropout_rate=dropout_rate, is_training=is_training)
